...
class config:
    ...
    def __init__(self):
        ...
        secret_name = (
            f"projects/{self.gcpProject}/secrets/{API_KEY_NAME}/versions/latest"
        )
        response = secret_client.access_secret_version(request={"name": secret_name})
        self.apiKey = response.payload.data.decode("UTF-8")
        # self.app = response.payload.data.decode("UTF-8")
        self.apiUser = os.environ.get("apiUser", "")
        ...
        self.gcpDnsZoneName = os.environ.get(
            "dnsZoneName", '"dnsZoneName" variable has not been set.'
        )
        self.gcpDnsDomain = os.environ.get(
            "dnsDomain", '"dnsDomain" variable has not been set.'
        )
