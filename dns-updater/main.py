...
if len(cfg.gcpAuthKeyJsonFile) == 0:
    credentials, project = google.auth.default(scopes="https://www.googleapis.com/auth/ndev.clouddns.readwrite")
else:
...
def main(request):
    ...
    if not (validIPv4Address(caller_ip)):
        logging.info(f"Cannot determine caller IP or wrong format {caller_ip}")
        caller_ip=""
